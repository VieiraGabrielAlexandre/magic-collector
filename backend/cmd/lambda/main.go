...
func main() {
	dsn := fmt.Sprintf(
		"%s:%s@tcp(%s:%s)/%s?parseTime=true&charset=utf8mb4&collation=utf8mb4_unicode_ci&timeout=10s&readTimeout=30s&writeTimeout=30s",
		getenv("DB_USER", "root"),
		getenv("DB_PASSWORD", ""),
	...
	// Lambda reutiliza o processo entre invocações — limita pool para evitar
	// esgotar conexões no MySQL externo.
	...
	// algnhsa converte eventos Lambda Function URL (formato HTTP API v2)
	// para chamadas net/http que o Gin processa normalmente.
